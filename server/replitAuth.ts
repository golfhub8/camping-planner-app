// Replit Auth integration - handles user authentication using OpenID Connect
...
// Cache the OIDC configuration for 1 hour to avoid repeated lookups
...
export function getSession() {
  ...
  // Store sessions in PostgreSQL database (required for Replit Auth)
  ...
  return session({
    ...
    cookie: {
      httpOnly: true,
      // Only require HTTPS in production (dev uses HTTP)
...
// Update user session with new OAuth tokens
...
export async function setupAuth(app: Express) {
  ...
  // Verify function called after successful OAuth authentication
  ...
  // Login route - redirects to Replit's OAuth login page
  ...
  // OAuth callback route - handles redirect after successful login
  ...
  // Logout route - clears session and redirects to Replit logout
...
// Usage: app.get("/api/protected", isAuthenticated, async (req, res) => {...})
