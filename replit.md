...
- **Security:** SSRF protection for recipe scraping endpoints (DNS resolution, private IP blocking, HTTPS enforcement). Recipe sharing endpoints sanitize public responses. Token generation includes collision detection and regeneration. CORS configured to allow all origins in development mode (for testing), and restrict to trusted origins in production: production domain (https://app.thecampingplanner.app), Vercel preview deployments (https://*.vercel.app with secure hostname validation), and local development (http://localhost:5173).
